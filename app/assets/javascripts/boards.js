  ...
  $("#shareModal").on("shown.bs.modal", () => {
    ...
    $.get("/api/friends", { board_id: boardSelector }, (data) => {
      ...
      for (var i = 0; i < data.length; i++) {
        var friend = data[i];
        $("#shareModal #list-friends").append(
          '<li class="list-group-item d-flex align-items-center"><span class="mr-auto"><strong>' +
            friend.first_name +
            " " +
            friend.last_name +
            "</strong>(" +
            friend.email +
            ')</span><button data-user-selected="false" data-user-id="' +
            friend.id +
            '" data-user-index-id="' +
            i +
            '" class="btn btn-primary">Add</button></li>'
        );
      }
    });
    ...
    $("#shareModal #list-friends").on("click", "button", () => {
      ...
      if (!isSelected) {
        ...
        var tag =
          "<div data-user-id='" +
          friends[index].id +
          "' class='tag mx-1'><span data-user-badge='" +
          friends[index].id +
          "' class='badge badge-secondary'>" +
          friends[index].first_name +
          " <button class='delete' data-user-id='" +
          friends[index].id +
          "'>x</button></span></div>";
    ...
    $("#share-board-button").on("click", () => {
      var ids = selectedFriends.map((value) => {
        return value.id;
      });
      var board_id = $(this).data("board-id");
      $.post("/api/boards/share", { ids: ids, board_id: board_id }).done(
        function (data) {
